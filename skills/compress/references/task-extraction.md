...
Guidance for identifying tasks in a session conversation.
...
- **Project next steps** — if it's something the user would naturally do the next time they work on that project, it belongs in the project file, not the task manager. The project file IS the tracking system.
...
Always include in task notes (if task manager supports notes):
```
