...
description: Create new Obsidian project files and folders. ALWAYS use this skill — not just when the user explicitly says "create a project", but any time the intent is to START TRACKING something new. This includes direct requests ("create a project for X", "/project X", "make a project", "set up a project") AND indirect signals ("this should be a project", "let's track this", "let's make it a project", "set up tracking for X", "we should have a project for this"). Also trigger when the user describes a new initiative with enough complexity to warrant tracking — school accounts, client engagements, multi-step builds, automations. If someone says "start a project" or "track this as a project" in ANY phrasing, this skill handles it. Do NOT trigger for loading/opening existing projects (that's project-load), updating project content, or archiving — only for CREATING new ones.
version: 2.0.0
...
- Sales prospects / school accounts (will have dossiers, quotes, meeting notes)
...
- **Tags:** infer from context (e.g., `[work, umbrella, sales]` for a sales project, `[project, heart, automation]` for a Heart automation)
...
Example: "Twin Cities German Immersion" → `twin-cities-german-immersion`
...
- **Things3 Tasks** — add when tasks have already been created
- **Resources** — add when there are known external links
- **Dataview queries** — add only if the user asks for them
...
Open the new project file in Obsidian. Requires `dangerouslyDisableSandbox: true`.
...
`{vault_name}` comes from crystal.local.yaml.
