---
name: project-load
description: Load, list, and file into existing Obsidian projects. ALWAYS use this skill when the user references an existing project by name — whether to load its context, check its status, file something into it, or list all projects. This includes direct commands ("/project-load X", "load project X", "show me project X") AND natural working phrases ("let's work on the X project", "pull up X", "switch to X project", "where are we on X"). Also trigger when FILING content into a project ("put this in project X", "file this under X", "save this to the X project"). Also trigger for listing projects ("what projects do we have", "show me all active projects"). Do NOT trigger for CREATING new projects (that's the project skill) — only for working with existing ones.
version: 1.0.0
