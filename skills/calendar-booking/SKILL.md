...
Query the calendar across the full timeframe using whatever calendar tool or integration is available.

Read `references/availability-rules.md` for the user's recurring patterns, time zone, and slot preferences — these shape which slots to propose.

Key rules (details in the reference file):
- Filter to the user's calendars only — the list of approved calendars should be defined in the user's integration state or config. Exclude any calendars not in that list (shared resource calendars, public calendars, etc.)
- Convert all times to the user's local timezone before presenting
- Use the calendar skill/tool appropriate to your setup (MCP iCal, GWS, etc.)
...
- /write handles creating the email draft
...
- Create the event on the appropriate calendar using the calendar tool
...
- Note relevant context ("Thursday morning is open but you have an on-site commitment in the afternoon")

### Step 5: Follow-Up Task (Optional)

When the scheduling requires a response from the other party, create a follow-up task in the user's task manager if one is configured:
...
Skip this step if no task management integration is available.
...
- "I always meet Kim at the office" -> Person/Org-Specific: meetings with Kim at office
